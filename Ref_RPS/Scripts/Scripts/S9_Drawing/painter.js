...
function Start (){
	baseTex = new Texture2D(Screen.width/2, Screen.height/2);
	for (var y : int = 0; y < baseTex.height; ++y) {
			for (var x : int = 0; x < baseTex.width; ++x) {
...
function OnGUI () {
	...
	GUILayout.BeginArea (Rect (0,0,baseTex.width*zoom,baseTex.height*zoom),"","Box");
...
function Update () {
	...
	if (Input.GetKeyDown ("mouse 0")) {
		...
		if (imgRect.Contains (mouse)) {
			if (tool==Tool.Vector) {
				var m2:Vector2 = mouse-Vector2(imgRect.x,imgRect.y);
				m2.y = imgRect.height-m2.y;
				var bz = new Array (BezierPoints);
...
function test () {
	...
	var w:int = 100;
	var h:int = 100;
	var p1:BezierPoint = new BezierPoint (Vector2(10,0),Vector2(5,20),Vector2(20,0));
	var p2:BezierPoint = new BezierPoint (Vector2(50,10),Vector2(40,20),Vector2(60,-10));
	var c:BezierCurve = new BezierCurve (p1.main,p1.control2,p2.control1,p2.main);
	p1.curve2=c;
	p2.curve1=c;
