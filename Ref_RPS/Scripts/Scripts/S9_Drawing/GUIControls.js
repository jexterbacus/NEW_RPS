...
static function RGBCircle (c : Color,label : String,colorCircle : Texture2D) {
 	var r: Rect  = GUILayoutUtility.GetAspectRect (1);
	print (r);
	r.height = r.width -= 15;
	var r2:Rect = Rect (r.x + r.width +5,r.y,10,r.height);
	var hsb:HSBColor = new HSBColor (c);//It is much easier to work with HSB colours in this case
	
	
	var cp:Vector2 = Vector2 (r.x+r.width/2,r.y+r.height/2);
	...
	if (Input.GetMouseButton (0)) {
		...
		var hyp:float = Mathf.Sqrt( (InputVector.x * InputVector.x) + (InputVector.y * InputVector.y) );
		if (hyp <= r.width/2 + 5) {
	...
	var hsb2:HSBColor = new HSBColor (c);
	hsb2.b = 1;
	var c2:Color = hsb2.ToColor ();
	GUI.color = c2;
	...
	var pos:Vector2 = (Vector2 (Mathf.Cos (hsb.h*360*Mathf.Deg2Rad),-Mathf.Sin (hsb.h*360*Mathf.Deg2Rad))*r.width*hsb.s/2);
